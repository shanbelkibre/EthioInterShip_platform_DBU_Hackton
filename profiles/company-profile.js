document.addEventListener("DOMContentLoaded", () => {
  ...
  profileForm.addEventListener("submit", function (e) {
    ...
    const logo =
      localStorage.getItem(`companyLogo_${currentUser.id}`) ||
    ...
    const updatedProfile = {
      ...
      specializations: Array.from(tagsList.children).map((tag) =>
  ...
  function addTag(tagText) {
    if (
      Array.from(tagsList.children).some(
        (tag) => tag.textContent.replace("×", "").trim() === tagText,
      )
    ) {
      return;
    }
