...
function loadDashboardStats(studentId) {
    setTimeout(() => {
        ...
        internships.forEach(internship => {
            const application = internship.applicants.find(app => app.studentId === studentId);
            if (application) {
                applicationsSent++;
        ...
        document.getElementById('accepted-applications').textContent = acceptedApplications;
...
function loadRecommendedInternships(studentId) {
    setTimeout(() => {
        let internships = JSON.parse(localStorage.getItem('internships')) || [];
        const studentProfile = JSON.parse(localStorage.getItem('studentProfiles'))?.find(profile => profile.id === studentId);
        ...
        if (studentProfile?.skills?.length > 0) {
            recommendedInternships = activeInternships.sort((a, b) => {
        ...
        internshipList.innerHTML = recommendedInternships.map(internship => {
            const deadline = new Date(internship.deadline);
            const daysLeft = Math.ceil((deadline - new Date()) / (1000 * 60 * 60 * 24));
            const hasApplied = internship.applicants.some(app => app.studentId === studentId);
