document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentUser) {
        ...
        const studentProfile = JSON.parse(localStorage.getItem('studentProfiles'))?.find(
        ...
        if (studentProfile) {
            ...
            document.getElementById('graduationYear').value = studentProfile.graduationYear || '';
        }
    }
...
function loadInternshipDetails(internshipId) {
    ...
    if (internship) {
        document.getElementById('internship-title').textContent = `Applying for: ${internship.title} at ${internship.company}`;
    } else {
        window.location.href = '../internships/internship-view.html';
    }
}
